...
# Path to the bundled admin1 names data file
_ADMIN1_NAMES_FILE = Path(__file__).parent / "data" / "admin1_names.json"


def _load_admin1_names() -> Dict[str, str]:
    """Load bundled admin1 region name mapping.

    Returns a dict mapping ``"CC.CODE"`` keys (e.g. ``"US.CA"``, ``"PL.78"``)
    to human-readable region names in the region's local/official language
    (e.g. ``"California"``, ``"Mazowieckie"``).

    The data file is generated by ``scripts/generate_admin1_names.py``.
    An empty dict is returned when the file is absent or unreadable, in which
    case the caller falls back to raw admin1 codes.
    """
    if not _ADMIN1_NAMES_FILE.exists():
        return {}
    try:
        with open(_ADMIN1_NAMES_FILE, encoding="utf-8") as fh:
            data = json.load(fh)
...
def load_region_catalog() -> RegionCatalog:
    ...
    # Load bundled admin1 region names (local/official language names).
    # Populated by scripts/generate_admin1_names.py; falls back gracefully to {}
    # when the file is absent, allowing the existing fallback chain to apply.
        ...
        try:
            ...
            if iso2 and iso2 in admin1_bbox:
                for a1code, box in admin1_bbox[iso2].items():
                    ...
                    #   1. Bundled admin1_names.json  – local/official language names
                    #   2. geonamescache get_subdivisions() – available in future versions
                    #   3. Raw admin1 code            – last-resort fallback
                    sub = subdivisions.get(key) if isinstance(subdivisions, dict) else None
                    state_name = (
                        admin1_names.get(key)
                        or (sub.get("name") if isinstance(sub, dict) else None)
                        or a1code
                    )
                    states[state_name] = (
                        float(box[0]),
